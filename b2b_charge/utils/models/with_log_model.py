...
class WithLogModel(BaseModel):
    histories = models.JSONField(default=default_histories, blank=True, null=True)
...
def m2m_signal_handler(sender, **kwargs):
    ...
    if "histories" not in model_fields:
        return
    ...
    if action in ("pre_remove", "pre_add", "pre_clear"):
        ...
        instance.__class__.objects.filter(pk=instance.id).update(histories=new_history)


def prepare_pre_remove_history(
    instance, m2m_field: str, pk_set: dict = {}
) -> Tuple[list, list]:
    pre_change = [o.__str__() for o in getattr(instance, m2m_field).all()]
    post_change = [
        o.__str__() for o in getattr(instance, m2m_field).exclude(pk__in=pk_set)
    ]
...
def prepare_pre_add_history(
    ...
    post_change = [
        o.__str__()
        for o in (
            getattr(instance, m2m_field).all()
...
def prepare_pre_clear_history(instance, m2m_field: str) -> Tuple[list, list]:
    pre_change = [o.__str__() for o in getattr(instance, m2m_field).all()]
    post_change = []
