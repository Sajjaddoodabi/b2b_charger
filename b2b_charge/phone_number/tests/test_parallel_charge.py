...
class ParallelChargeTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="vendor_user", password="testpass")
    ...
    def charge_phone(self, phone, amount=Decimal("5.00")):
        ...
        try:
            with db_transaction.atomic():
                print(f"Locking phone and vendor {phone.number} | Transfer ID: {transfer_id}")
                locked_phone = PhoneNumber.objects.select_for_update().get(pk=phone.pk)
                locked_vendor = Vendor.objects.select_for_update().get(pk=self.vendor.pk, is_active=True)
                print(f"Locked phone and vendor {phone.number} | Transfer ID: {transfer_id}")
    ...
    def test_parallel_charges(self):
        ...
        print(f"Final vendor balance: {self.vendor.balance} (Expected: {expected_balance})")
        self.assertEqual(
            self.vendor.balance,
            expected_balance,
